-- Modified by Sriram Sankar to get file names to DOS conventions.
...
-- END OF UMASS CODES.
	...
	Base, Base5 : STR(Max_Name_Length);
    begin
	...
	if Input_File'Length <= 7 then
           Assign(Input_File(Input_File'First..Input_File'Last-2),To=>Base5);
	else
           Assign(Input_File(Input_File'First..Input_File'First+4),To=>Base5);
	end if;
	...
	Assign(Base,   To => Verbose_File_Name);
        Append(".vrb", To => Verbose_File_Name); 

	Assign(Base5,        To => Tokens_File_Name);
	Append("_tk" & Extension, To => Tokens_File_Name); 

-- UMASS CODES :
	Assign(Base,        To => Error_Report_File_Name);
        Append("_er" & Extension, To => Error_Report_File_Name); 

	Assign(Base,        To => Listing_File_Name);
        Append(".lis", To => Listing_File_Name); 
-- END OF UMASS CODES.

	Assign("yyparse.tpl", To => Template_File_Name);

	Assign(Base,   To => Actions_File_Name);
	Append(".acs", To => Actions_File_Name);

	Assign(Base5,              To => Shift_Reduce_File_Name);
	Append("_sr" & Extension, To => Shift_Reduce_File_Name);

	Assign(Base5,      To => Goto_File_Name);
	Append("_gt" & Extension, To => Goto_File_Name);

	Assign(Base5,       To => C_Lex_File_Name);
	Append("_cl" & Extension, To => C_Lex_File_Name);
